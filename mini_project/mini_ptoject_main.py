...
def get_cfg_funcs(proj, binary, excluded):
    ...
    return list(filter(None, [f if f.binary_name == binary and (not f.is_plt) and not f.name.startswith(
        "sub_") and f.name not in excluded else None for f in
...
def generate_dataset(train_binaries, output_name, dataset_name): #keep
    ...
    for binary in train_binaries:
        analysed_funcs = analyse_binary(analysed_funcs, binary, output_dir)
...
def analyse_binary(analysed_funcs, binary_name, dataset_dir): #keep
    ...
    for test_func in funcs:
        ...
        filename= os.path.join(binary_dir, f"{test_func.name}")
        if(os.path.isfile(filename)):
            counter = 0
            while os.path.isfile(filename + "*" + str(counter)):
                counter += 1
            filename = os.path.join(binary_dir, f"{test_func.name}") + "*" + str(counter)
...
def main():
    ...
    # Below is debug line for mtools exe
    #analyse_binary(set(), "our_dataset/nero_dataset/gcc-5__Ou__mtools__mtools", "debug_mtools") 
